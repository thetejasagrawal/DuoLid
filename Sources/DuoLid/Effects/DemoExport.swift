...
enum DemoExport {
    static func run() throws {
        let output = URL(fileURLWithPath: FileManager.default.currentDirectoryPath).appendingPathComponent("artifacts/demo")
        ...
        try pixels.withUnsafeMutableBytes { bytes in
            guard let context = CGContext(data: bytes.baseAddress, width: width, height: height, bitsPerComponent: 8,
                bytesPerRow: width * 4, space: CGColorSpace(name: CGColorSpace.sRGB)!, bitmapInfo: bitmap.rawValue) else { throw RenderError.unavailable }
        ...
        settings.glowEnabled = true; settings.edgeBleed = 0.8
        ...
        let metadata = "{\"source\":\"synthetic PreviewDesktop\",\"renderer\":\"production MetalRenderer\",\"fps\":60,\"frames\":240,\"liveCapture\":false,\"performanceEvidence\":false}\n"
    ...
    private static func png(_ texture: MTLTexture) throws -> Data {
        ...
        guard let provider = CGDataProvider(data: Data(bytes) as CFData),
              ...
              let data = NSBitmapImageRep(cgImage: image).representation(using: .png, properties: [:]) else { throw RenderError.unavailable }
        return data
    }
}
