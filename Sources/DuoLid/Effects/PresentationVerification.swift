...
enum PresentationVerification {
    static func run() throws {
        guard CommandLine.arguments.contains("--allow-visible-test") else {
            FileHandle.standardError.write(Data("Visible diagnostics are disabled. On a designated test Mac, add --allow-visible-test for a windowed test; --full-screen is a separate opt-in.\n".utf8))
            exit(64)
        }
...
private final class PresentationSession: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    init(arguments args: [String] = CommandLine.arguments) throws {
        guard let screen = DesktopEffect.builtInScreen, let displayID = screen.displayID else { throw RenderError.unavailable }
        ...
        guard [30, 60, 120].contains(requested) else { throw NSError(domain: "DuoLid.Verification", code: 64, userInfo: [NSLocalizedDescriptionKey: "Use --fps 30, 60, or 120."]) }
        ...
        let contentRect = fullScreen ? screen.frame : CGRect(x: screen.visibleFrame.midX - 480,
            y: screen.visibleFrame.midY - 310, width: 960, height: 620)
        window = NSWindow(contentRect: contentRect, styleMask: fullScreen ? .borderless : [.titled, .closable], backing: .buffered, defer: false)
        ...
        layer.device = renderer.device; layer.pixelFormat = .bgra8Unorm
        layer.maximumDrawableCount = 2; layer.framebufferOnly = true
        layer.allowsNextDrawableTimeout = true
        layer.presentsWithTransaction = false; layer.displaySyncEnabled = true
        layer.contentsScale = screen.backingScaleFactor
        // Keep full native GPU workload even when the diagnostic window is small.
        layer.drawableSize = CGSize(width: width, height: height)
        layer.colorspace = CGColorSpace(name: CGColorSpace.sRGB)
        view.layer = layer; view.wantsLayer = true; window.contentView = view
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        escapeMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            if event.keyCode == 53 { MainActor.assumeIsolated { self?.finish(cancelled: true) }; return nil }
            return event
        }
    ...
    private func startCapture() async throws {
        ...
        guard !finishing, !Task.isCancelled else { return }
        guard let display = content.displays.first(where: { $0.displayID == displayID }) else { throw RenderError.unavailable }
        ...
        configuration.queueDepth = 4; configuration.showsCursor = false
        configuration.capturesAudio = false; configuration.colorSpaceName = CGColorSpace.sRGB
        ...
        try stream.addStreamOutput(receiver, type: .screen, sampleHandlerQueue: DispatchQueue(label: "app.duolid.diagnostic.capture", qos: .userInteractive))
        self.receiver = receiver; self.stream = stream
    ...
    private func finish(cancelled: Bool) {
        ...
        window.orderOut(nil)
        timer?.invalidate(); timer = nil; timeoutTask?.cancel()
        let startup = startupTask; startup?.cancel()
        ...
        Task { [self] in
            ...
            struct RunReport: Encodable { let mode: String; let cancelled: Bool; let drained: Bool; let timing: PerformanceReport }
            let encoder = JSONEncoder(); encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
            if let data = try? encoder.encode(RunReport(mode: live ? "live-capture" : "synthetic", cancelled: cancelled, drained: drained, timing: report)) {
    ...
    func windowShouldClose(_ sender: NSWindow) -> Bool { finish(cancelled: true); return false }
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply { finish(cancelled: true); return .terminateCancel }

    private func update() {
        guard !finishing else { return }
        if renderer.statistics.isStalled(at: CACurrentMediaTime()) { finish(cancelled: true); return }
        var settings = DuoSettings(); settings.glowEnabled = true
    ...
    private static func syntheticBuffer(width: Int, height: Int) throws -> CVPixelBuffer {
        ...
        let bytes = data.assumingMemoryBound(to: UInt8.self), stride = CVPixelBufferGetBytesPerRow(buffer)
        for y in 0..<height {
            for x in 0..<width {
                let i = y * stride + x * 4
