...
Animation and command-buffer presentation use the same display-clock target. This follows Apple's [`targetTimestamp`](https://developer.apple.com/documentation/quartzcore/cadisplaylink/targettimestamp) and [`present(_:atTime:)`](https://developer.apple.com/documentation/metal/mtlcommandbuffer/present(_:attime:)) APIs; source changes alone do not establish that a scheduling issue is fixed.
