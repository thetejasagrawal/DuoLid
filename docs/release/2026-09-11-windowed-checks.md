...
Cleanup review found two separate conservative-retirement gaps: an unavailable or failed Metal fence must return an undrained result, and retaining only a hidden desktop panel does not preserve its renderer's working resources. The renderer now releases its working resources only after a successfully completed fence. On failure, the desktop path retains the panel, renderer, and render loop in a process-lifetime quarantine, pauses graphics, persists the interrupted-session marker, and rejects retries until relaunch. The preview path already retains its view and renderer together. These changes handle failure cases without claiming to explain the pink-screen incident.

The final follow-up source is `5e02f98df405bd16204af1f3201aa4e078c4d377`. All 37 local behavioral tests and strict Swift 6 compilation passed. Both Apple silicon and Intel [CI jobs](https://github.com/thetejasagrawal/DuoLid/actions/runs/34527006636) also passed strict tests, universal development packaging, script/site checks, and the diagnostic opt-in guard for this revision. A clean Developer ID build passed universal architecture, macOS 14 target, nested-signature, hardened-runtime, compiled-shader, resource, updater-policy, and matching-symbol verification. It remains a local candidate; the installed application was not replaced. No graphics test was run after these cleanup changes.
