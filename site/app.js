...
playButton.addEventListener("click", async () => {
  if (video.paused) {
    try {
      ...
      playButton.textContent = "Use the video controls to play";
    }
  } else video.pause();
});
...
async function loadRelease() {
  try {
    ...
    if (
      ...
      !/^\d+\.\d+\.\d+(?:-beta\.\d+)?$/.test(release.version) ||
      release.download !== expectedDownload ||
      release.notes !== expectedNotes
    )
      return;
    ...
    document.querySelector("#release-detail").textContent =
      `v${release.version} · macOS 14+ · Apple silicon & Intel`;
    ...
    document.querySelector("#availability-answer").textContent =
      `DuoLid ${release.version} is available. Use the download button above. Review the compatibility notes before installing; ${release.beta ? "this is a beta release." : "hardware sensor support varies."}`;
  } catch {
